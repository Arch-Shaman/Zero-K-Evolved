...
function reloadThread()
	while true do
		if reloading then
			Sleep(reloadtime)
			reloading = false
		elseif tracking >= trackneeded then
			local x,y,z = Spring.GetUnitPosition(unitID)
			Spring.PlaySoundFile("trackercompleted.wav",1.0,x,y,z,1,1,1,1)
			Sleep(570)
			trackingcomplete = true
		end
		if oldtime == tracking and tracking > 0 and tracking < trackneeded then
			tracking = tracking - 4
			if tracking < 0 then
				tracking = 0
			end
