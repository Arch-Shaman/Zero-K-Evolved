...
function gadget:Explosion(weaponDefID, px, py, pz, AttackerID, projectileID)
	...
	if config[weaponDefID] == 'targeter' then
		...
		if debug then
			local x,y,z = spGetUnitPosition(missiles[AttackerID].target)
...
function gadget:ProjectileCreated(proID, proOwnerID, weaponDefID) -- proOwnerID is the unitID that fired the projectile
	...
	if config[weaponDefID] and not missiles[proOwnerID] then
		...
		--movectrlOn(fakeid)
		missiles[proOwnerID] = {missiles = {}, target = fakeid, numMissiles = 0, lastframe = spGetGameFrame(), state = "normal"}
		if debug then
			local x,y,z = spGetUnitPosition(fakeid)
			debugecho("created fake unit " .. fakeid .. " Currentpos: " .. tostring(x,y,z))
		end
	end
	if config[weaponDefID] and missiles[proOwnerID].missiles[proID] then
		return
	end
	...
	if config[weaponDefID] and config[weaponDefID] == 'tracker' then
		...
		local success = spSetProjectileTarget(proID, missiles[proOwnerID].target, byte('u'))
	...
	if f%5 == 3 then
		for id, data in pairs(missiles) do
			...
			elseif f - data.lastframe < 10 and data.state ~= "normal" and data.numMissiles > 0 then
				data.state = "normal"
				for pid,_ in pairs(data.missiles) do
					local success = spSetProjectileTarget(pid, data.target, byte('u'))
					debugecho("retargeting: " .. tostring(success))
				end
			end
